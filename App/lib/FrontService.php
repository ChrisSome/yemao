...
class  FrontService {
    ...
    public static function handInformationComment($informationComments, $authId)
    {
        ...
        if ($parentCommentIds) {
            ...
            $formatParentComments = [];
        }

        foreach ($informationComments as $v) {
            $parentCommentIds[] = $v['parent_id'];
            $id = intval($v['id']);
            ...
            $list[] = [
                ...
                'parent_content' => isset($formatParentComments[$v['parent_id']]) ? $formatParentComments[$v['parent_id']] : null,
                'created_at' => $v['created_at'],
                'respon_number' => $v['respon_number'],
                'fabolus_number' => $v['fabolus_number'],
     ...
     * @param int $isNewStatus   是否用最新的status_id
     * @return array
    ...
    static function formatMatchThree($matches, $uid, $interestMatchArr)
    {
        ...
        foreach ($matches as $match) {
            ...
            $is_start = false;
            if (in_array($match->status_id, FootballApi::STATUS_PLAYING)) {
                $is_start = true;
            }
            $home_win = 0;
...
//            $cacheStatus = Cache::get('football-match-status-' . $match->match_id);
            ...
            $steamLink = isset($formatSteams[$match->match_id]) ? $formatSteams[$match->match_id] : "";
            if (!isset($formatTeams[$match->home_team_id]) || !isset($formatTeams[$match->away_team_id])) continue;
            ...
            $item['competition_name'] = $formatCompetition[$match->competition_id]['short_name_zh'];
            $item['competition_color'] = $formatCompetition[$match->competition_id]['primary_color'];
