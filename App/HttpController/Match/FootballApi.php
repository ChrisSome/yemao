...
//        $sensus = AdminClashHistory::getInstance()->where('match_id', $this->params['match_id'])->get();
        ...
        if (!$currentSeasonId) {
            ...
            if ($decode) {
                foreach ($decode as $item) {
                    if (!isset($item['rows'])) continue;
                    array_walk($item['rows'], function($v, $k) use ($homeTeamId, $awayTeamId, &$homeIntvalRank, &$awayIntvalRank) {
        ...
        $formatRelateMatch = FrontService::formatMatchThree($relateMatch, $userId, $userIntereMatchIds);
        $formatHistoryMatches = $homeRecentMatches = $awayRecentMatches = $homeRecentSchedule = $awayRecentSchedule = [];
        foreach ($formatRelateMatch as $itemMatch)
        {
            if (!isset($itemMatch['home_team_id'])) {
            }
            if ($itemMatch['status_id'] == 8) {
                //历史交锋
            ...
            } else if (in_array($itemMatch['status_id'], array_merge([1], self::STATUS_PLAYING))) {
                if ($itemMatch['home_team_id'] == $homeTid || $itemMatch['away_team_id'] == $homeTid) {
                    $homeRecentSchedule[] = $itemMatch;
                }
                if ($itemMatch['home_team_id'] == $awayTid || $itemMatch['away_team_id'] == $awayTid) {
                    $awayRecentSchedule[] = $itemMatch;
                }
            }
        }
...
//            'historyResult' => !empty($sensus['history']) ? json_decode($sensus['history'], true) : null,//历史交锋统计
//            'recentResult' => !empty($sensus['recent']) ? json_decode($sensus['recent'], true) : null, //近期战绩
            'history' => $formatHistoryMatches ? array_slice($formatHistoryMatches, 0, 10) : null,//历史交锋比赛
            'homeRecent' => array_slice($homeRecentMatches, 0, 10),//主队近期赛果
            'awayRecent' => array_slice($awayRecentMatches, 0, 10),//客队近期赛果
            'homeRecentSchedule' => array_slice($homeRecentSchedule, 0, 10),//主队近期赛程
            'awayRecentSchedule' => array_slice($awayRecentSchedule, 0, 10),//客队近期赛程
        ];
